...
export function ImportDialog({ open, onClose, onImport }: ImportDialogProps) {
  ...
  const handleTextImport = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/pool-dooks/import", {
        method: "POST",
  ...
  const handleImageScan = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/ai/scan-image", { method: "POST", body: form });
      const data = await res.json();
  ...
  const handleFileImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res = await fetch("/api/pool-dooks/import", { method: "POST", body: form });
      if (!res.ok) throw new Error("Import failed");
      const data = await res.json();
      onImport({ bodyJson: data.bodyJson as JSONContent });
    } catch {
